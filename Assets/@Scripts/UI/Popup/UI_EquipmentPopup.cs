...
public class UI_EquipmentPopup : UI_Popup
{
    ...
    void Refresh()
    {
        ...
        foreach (Equipment item in Managers._Game.OwnedEquipments)
        {
            //착용중인장비 
            if (item.IsEquipped)
            {
    ...
    void OnClickCharacterButton() // 캐릭터 버튼
    {
        ...
        //Managers._UI.ShowPopupUI<UI_CharacterSelectPopup>();
    }
    ...
    public void SetItem()
    {
        ...
        foreach (int id in Managers._Game.ItemDictionary.Keys)
        {
            if (Managers._Data.MaterialDic.TryGetValue(id, out MaterialData material) == true)
            {
                UI_MaterialItem item = Managers._UI.MakeSubItem<UI_MaterialItem>(Container.transform);
                int count = Managers._Game.ItemDictionary[id];
