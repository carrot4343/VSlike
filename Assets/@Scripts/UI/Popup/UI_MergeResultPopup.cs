...
public class UI_MergeResultPopup : UI_Popup
{
    ...
    private void OnEnable()
    {
        ...
        //Managers._Sound.Play(Define.Sound.Effect, "Result_CommonMerge");
    ...
    void Refresh()
    {
        ...
        #region 기본 정보
        // EquipmentImage : 장비의 아이콘
        Sprite spr = Managers._Resource.Load<Sprite>(_afterEquipment.EquipmentData.SpriteName);
        GetImage((int)Images.EquipmentTypeImage).sprite = Managers._Resource.Load<Sprite>($"{_afterEquipment.EquipmentData.EquipmentType}_Icon.sprite");
        ...
        #region 옵션
        // 최대 레벨
        GetText((int)Texts.BeforeLevelValueText).text = $"{Managers._Data.EquipDataDic[_beforeEquipment.EquipmentData.DataId].MaxLevel}"; // 합성 전 공격력
        GetText((int)Texts.AfterLevelValueText).text = $"{Managers._Data.EquipDataDic[_beforeEquipment.EquipmentData.DataId].MaxLevel}"; // 합성 전 공격력
        ...
        if (_beforeEquipment.EquipmentData.AtkDmgBonus != 0) // 공격력이 0이 아니면 무기
        {
            ...
            GetText((int)Texts.BeforeATKValueText).text = $"{Managers._Data.EquipDataDic[_beforeEquipment.EquipmentData.DataId].MaxLevel}"; // 합성 전 공격력
            GetText((int)Texts.AfterATKValueText).text = $"{Managers._Data.EquipDataDic[_afterEquipment.EquipmentData.DataId].MaxLevel}"; // 합성 후 공격력
        }
        else
        {
            // 체력 옵션
            ...
            GetText((int)Texts.BeforeHPValueText).text = Managers._Data.EquipDataDic[_beforeEquipment.EquipmentData.DataId].MaxLevel.ToString(); // 합성 전 체력
            GetText((int)Texts.AfterHPValueText).text = Managers._Data.EquipDataDic[_afterEquipment.EquipmentData.DataId].MaxLevel.ToString(); // 합성 후 체력 
        }
        #endregion
    }
