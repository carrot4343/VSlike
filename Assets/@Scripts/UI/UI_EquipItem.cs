...
public class UI_EquipItem : UI_Base
{
    ...
    public void SetInfo(Equipment item, UI_ItemParentType parentType, ScrollRect scrollRect = null)
    {
        ...
        // EquipmentImage : 장비의 아이콘
        Sprite spr = Managers._Resource.Load<Sprite>(Equipment.EquipmentData.SpriteName);
        GetImage((int)Images.EquipmentImage).sprite = spr;
        // 장비 타입 아이콘
        Sprite tpy = Managers._Resource.Load<Sprite>($"{Equipment.EquipmentData.EquipmentType}_Icon.sprite");
    ...
    void OnClickEquipItemButton() // 장비 선택 시
    {
        ...
        if (_parentType == UI_ItemParentType.GachaResultPopup)
        {
            Managers._UI.ShowPopupUI<UI_GachaEquipmentInfoPopup>().SetInfo(Equipment);
        }
        else
        {
            ...
            // 해당 장비의 장비 정보 팝업을 호출
            UI_EquipmentInfoPopup infoPopup = (Managers._UI.SceneUI as UI_LobbyScene).EquipmentInfoPopupUI;
            if (infoPopup != null)
            {
