...
public class UI_MergePopup : UI_Popup
{
    ...
    void Refresh_SelectedEquipObject()
    {
        ...
        {
            GetImage((int)Images.SelectedEquipImage).sprite = Managers._Resource.Load<Sprite>(_equipment.EquipmentData.SpriteName);
            GetImage((int)Images.SelectedEquipTypeImage).sprite = Managers._Resource.Load<Sprite>($"{_equipment.EquipmentData.EquipmentType}_Icon.sprite");
    ...
    void Refresh_MergeEquip1()
    {
        ...
        {
            GetImage((int)Images.FirstSelectEquipImage).sprite = Managers._Resource.Load<Sprite>(_mergeEquipment1.EquipmentData.SpriteName);
            GetImage((int)Images.FirstSelectEquipTypeImage).sprite = Managers._Resource.Load<Sprite>($"{_mergeEquipment1.EquipmentData.EquipmentType}_Icon.sprite");
    ...
    void Refresh_MergeEquip2()
    {
        ...
        {
            GetImage((int)Images.SecondSelectEquipImage).sprite = Managers._Resource.Load<Sprite>(_mergeEquipment2.EquipmentData.SpriteName);
            GetImage((int)Images.SecondSelectEquipTypeImage).sprite = Managers._Resource.Load<Sprite>($"{_mergeEquipment2.EquipmentData.EquipmentType}_Icon.sprite");
    ...
    bool CheckEnableMergeButton()
    {
        ...
        GetImage((int)Images.SelectedEquipEnforceBackgroundImage).gameObject.SetActive(false);
        GetObject((int)GameObjects.OptionResultObject).gameObject.SetActive(true); // 옵션 결과
        GetText((int)Texts.EquipmentNameText).text = $"{Managers._Data.EquipDataDic[mergedItemId].NameTextID}"; // 이름
        GetText((int)Texts.BeforeLevelValueText).text = $"{Managers._Data.EquipDataDic[_equipment.EquipmentData.DataId].MaxLevel}"; // 합성 전 최고 레벨
        GetText((int)Texts.AfterLevelValueText).text = $"{Managers._Data.EquipDataDic[mergedItemId].MaxLevel}"; // 합성 후 최고 레벨

        if (Managers._Data.EquipDataDic[mergedItemId].AtkDmgBonus != 0) // 장비의 공격력이 0이 아니면 무기
        {
            // 공격력 옵션
            ...
            GetText((int)Texts.BeforeATKValueText).text = $"{Managers._Data.EquipDataDic[_equipment.EquipmentData.DataId].MaxLevel}"; // 합성 전 공격력
            GetText((int)Texts.AfterATKValueText).text = $"{Managers._Data.EquipDataDic[mergedItemId].MaxLevel}"; // 합성 후 공격력
        }
        else
        {
            // 체력 옵션
            ...
            GetText((int)Texts.BeforeHPValueText).text = Managers._Data.EquipDataDic[_equipment.EquipmentData.DataId].MaxLevel.ToString(); // 합성 전 체력
            GetText((int)Texts.AfterHPValueText).text = Managers._Data.EquipDataDic[mergedItemId].MaxLevel.ToString(); // 합성 후 체력 
        }

        // 등급별 처리
        switch (Managers._Data.EquipDataDic[mergedItemId].EquipmentGrade)
        {
            case EquipmentGrade.Uncommon:
                GetImage((int)Images.SelectedEquipGradeBackgroundImage).color = EquipmentUIColors.Uncommon;
                ...
                int uncommonGradeSkillId = Managers._Data.EquipDataDic[mergedItemId].UncommonGradeSkill;
                GetText((int)Texts.ImprovOptionValueText).text = $"+ {Managers._Data.SupportSkillDic[uncommonGradeSkillId].Description}"; // 추가 옵션
            ...
            case EquipmentGrade.Rare:
                ...
                int rareGradeSkillId = Managers._Data.EquipDataDic[mergedItemId].RareGradeSkill;
                GetText((int)Texts.ImprovOptionValueText).text = $"+ {Managers._Data.SupportSkillDic[rareGradeSkillId].Description}"; // 추가 옵션
            ...
            case EquipmentGrade.Epic:
                ...
                int epicGradeSkillId = Managers._Data.EquipDataDic[mergedItemId].EpicGradeSkill;
                GetText((int)Texts.ImprovOptionValueText).text = $"+ {Managers._Data.SupportSkillDic[epicGradeSkillId].Description}"; // 추가 옵션
            ...
            case EquipmentGrade.Legendary:
                ...
                int legendaryGradeSkillId = Managers._Data.EquipDataDic[mergedItemId].LegendaryGradeSkill;
                GetText((int)Texts.ImprovOptionValueText).text = $"+ {Managers._Data.SupportSkillDic[legendaryGradeSkillId].Description}"; // 추가 옵션
    ...
    void SortEquipments()
    {
        Managers._Game.SortEquipment(_equipmentSortType);

        GetObject((int)GameObjects.EquipInventoryScrollContentObject).DestroyChilds();

        // 장비 리스트 작성
        foreach (Equipment inventoryEquipmentItem in Managers._Game.OwnedEquipments)
        {
            ...
            UI_MergeEquipItem item = Managers._UI.MakeSubItem<UI_MergeEquipItem>(GetObject((int)GameObjects.EquipInventoryScrollContentObject).transform);
    ...
    void OnClickBackButton() // 되돌아가기 버튼
    {
        ...
        (Managers._UI.SceneUI as UI_LobbyScene).EquipmentPopupUI.SetInfo();
    }
    ...
    {
        // 자동 합성 버튼
        Managers._Game.SortEquipment(EquipmentSortType.Grade);

        Equipment[] equipments = Managers._Game.OwnedEquipments.ToArray();
        ...
        for (int i = 0; i < equipments.Length; i++)
        {
            ...
            for (int j = i; j < equipments.Length; j++)
            {
                if (equipments[j] != null)
                {
                    ...
                    {
                        Equipment newItem = Managers._Game.MergeEquipment(_equipment, _mergeEquipment1, _mergeEquipment2, true);
                        if (newItem != null)
        ...
        if (newEquipments.Count > 0)
        {
            ...
            Managers._UI.ShowPopupUI<UI_MergeAllResultPopup>().SetInfo(newEquipments);
        }
        Managers._Game.SaveGame();
    }
    ...
    void OnClickMergeButton()
    {
        ...
        Equipment newItem = Managers._Game.MergeEquipment(_equipment, _mergeEquipment1, _mergeEquipment2);

        UI_MergeResultPopup resultPopup = (Managers._UI.SceneUI as UI_LobbyScene).MergeResultPopupUI;
