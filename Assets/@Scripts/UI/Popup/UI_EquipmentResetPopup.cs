...
public class UI_EquipmentResetPopup : UI_Popup
{
    ...
    {
        GetImage((int)Images.TargetEquipmentImage).sprite = Managers._Resource.Load<Sprite>(_equipment.EquipmentData.SpriteName);
        GetText((int)Texts.TargetEquipmentLevelValueText).text = $"Lv. {_equipment.Level}";

        GetImage((int)Images.ResultEquipmentImage).sprite = Managers._Resource.Load<Sprite>(_equipment.EquipmentData.SpriteName);
        GetText((int)Texts.ResultGoldCountValueText).text = $"{CalculateResetGold()}";

        GetImage((int)Images.ResultMaterialImage).sprite = Managers._Resource.Load<Sprite>(Managers._Data.MaterialDic[_equipment.EquipmentData.LevelupMaterialID].SpriteName);
    ...
    {
        GetImage((int)Images.DowngradeTargetEquipmentImage).sprite = Managers._Resource.Load<Sprite>(_equipment.EquipmentData.SpriteName);
        GetText((int)Texts.DowngradeTargetEquipmentLevelValueText).text = $"Lv. {_equipment.Level}";

        GetImage((int)Images.DowngradeEquipmentImage).sprite = Managers._Resource.Load<Sprite>(_equipment.EquipmentData.SpriteName);
        GetText((int)Texts.DowngradeResultGoldCountValueText).text = $"{CalculateResetGold()}";

        GetImage((int)Images.DowngradEnchantStoneImage).sprite = Managers._Resource.Load<Sprite>(Managers._Data.EquipDataDic[_equipment.EquipmentData.DowngradeMaterialCode].SpriteName);
        GetText((int)Texts.DowngradEnchantStoneCountValueText).text = $"{_equipment.EquipmentData.DowngradeMaterialCount}";
        
        GetImage((int)Images.DowngradeResultMaterialImage).sprite = Managers._Resource.Load<Sprite>(Managers._Data.MaterialDic[_equipment.EquipmentData.LevelupMaterialID].SpriteName);
    ...
    int CalculateResetGold()
    {
        ...
        {
            gold += Managers._Data.EquipLevelDataDic[i].UpgradeCost;
        }
        return gold;
    }
    ...
    int CalculateResetMaterialCount()
    {
        ...
        for (int i = 1; i < _equipment.Level; i++)
        {
            materialCount += Managers._Data.EquipLevelDataDic[i].UpgradeRequiredItems;
        }
    ...
    void OnClickEquipmentResetButton()
    {
        ...
        spriteName[1] = Managers._Data.MaterialDic[materialCode].SpriteName;
        ...
        Managers._Game.ExchangeMaterial(Managers._Data.MaterialDic[Define.ID_GOLD], gold);
        Managers._Game.ExchangeMaterial(Managers._Data.MaterialDic[materialCode], materialCount);
        (Managers._UI.SceneUI as UI_LobbyScene).EquipmentInfoPopupUI.gameObject.SetActive(false);
    ...
    void OnClickEquipmentDowngradeButton()
    {
        ...
        if (Managers._Data.EquipDataDic.TryGetValue(_equipment.EquipmentData.DowngradeEquipmentCode, out Data.EquipmentData downgradedEquip) == false)
            return;
        ...
        Managers._Game.AddEquipment(downgradedEquip.DataId);
        ...
        {
            int materialCode = _equipment.EquipmentData.LevelupMaterialID;
            spriteNameList.Add(Managers._Data.MaterialDic[materialCode].SpriteName);
            count.Add(materialCount);
            Managers._Game.ExchangeMaterial(Managers._Data.MaterialDic[materialCode], materialCount);
        }
        
        spriteNameList.Add(Managers._Data.EquipDataDic[_equipment.EquipmentData.DowngradeMaterialCode].SpriteName);
