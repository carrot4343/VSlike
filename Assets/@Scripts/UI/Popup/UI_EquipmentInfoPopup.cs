...
public class UI_EquipmentInfoPopup : UI_Popup
{
    ...
    void Refresh()
    {
        ...
        GetImage((int)Images.EquipmentTypeImage).sprite = Managers._Resource.Load<Sprite>($"{_equipment.EquipmentData.EquipmentType}_Icon.sprite");
        GetImage((int)Images.EquipmentImage).sprite = Managers._Resource.Load<Sprite>(_equipment.EquipmentData.SpriteName);
        ...
        GetImage((int)Images.EquipmentOptionImage).sprite = Managers._Resource.Load<Sprite>(sprName);
        // EquipmentOptionValueText : 장비 옵션 수치
        ...
        // CostGoldValueText : 레벨업 비용 (보유 / 필요) 만약 코스트가 부족하다면 보유량을 빨간색(#F3614D)으로 보여준다. 부족하지 않다면 흰색(#FFFFFF)
        if (Managers._Data.EquipLevelDataDic.ContainsKey(_equipment.Level))
        {
            GetText((int)Texts.CostGoldValueText).text = $"{Managers._Data.EquipLevelDataDic[_equipment.Level].UpgradeCost}";
            if (Managers._Game.Gold < Managers._Data.EquipLevelDataDic[_equipment.Level].UpgradeCost)
                GetText((int)Texts.CostGoldValueText).color = Utils.HexToColor("F3614D");
            
            GetText((int)Texts.CostMaterialValueText).text = $"{Managers._Data.EquipLevelDataDic[_equipment.Level].UpgradeRequiredItems}";

        }

        // 레벨업 재료의 아이콘 #Neo
        GetImage((int)Images.CostMaterialImage).sprite = Managers._Resource.Load<Sprite>(Managers._Data.MaterialDic[_equipment.EquipmentData.LevelupMaterialID].SpriteName);
        #endregion
        ...
        if (Managers._Data.SupportSkillDic.ContainsKey(_equipment.EquipmentData.UncommonGradeSkill)) // 스킬타입에서 서포트스킬 타입 데이터로 교체 #Neo
        {
            SupportSkillData skillData = Managers._Data.SupportSkillDic[_equipment.EquipmentData.UncommonGradeSkill];
        ...
        if (Managers._Data.SupportSkillDic.ContainsKey(_equipment.EquipmentData.RareGradeSkill)) 
        {
            SupportSkillData skillData = Managers._Data.SupportSkillDic[_equipment.EquipmentData.RareGradeSkill];
        ...
        if (Managers._Data.SupportSkillDic.ContainsKey(_equipment.EquipmentData.EpicGradeSkill))
        {
            SupportSkillData skillData = Managers._Data.SupportSkillDic[_equipment.EquipmentData.EpicGradeSkill];
        ...
        if (Managers._Data.SupportSkillDic.ContainsKey(_equipment.EquipmentData.LegendaryGradeSkill))
        {
            SupportSkillData skillData = Managers._Data.SupportSkillDic[_equipment.EquipmentData.LegendaryGradeSkill];
    ...
    void OnClickEquipmentResetButton() // 장비 초기화 팝업 버튼
    {
        ...
        UI_EquipmentResetPopup resetPopup = (Managers._UI.SceneUI as UI_LobbyScene).EquipmentResetPopupUI;
    ...
    {
        //Managers._Sound.Play(Define.Sound.Effect, "Equip_Equipment");

        // 장비를 장착한다
        Managers._Game.EquipItem(_equipment.EquipmentData.EquipmentType, _equipment);
        Refresh();
    ...
    void OnClickUnequipButton() // 장착 해제 버튼
    {
        //Managers._Sound.PlayButtonClick();
        // 장비를 장착 헤제한다.
        Managers._Game.UnEquipItem(_equipment);
        Refresh();
    ...
    void OnClickLevelupButton() // 레벨업 버튼
    {
        ...
        int UpgradeCost = Managers._Data.EquipLevelDataDic[_equipment.Level].UpgradeCost;
        int UpgradeRequiredItems = Managers._Data.EquipLevelDataDic[_equipment.Level].UpgradeRequiredItems;
        ...
        int numMaterial = 0;
        Managers._Game.ItemDictionary.TryGetValue(_equipment.EquipmentData.LevelupMaterialID, out numMaterial);
       
        if (Managers._Game.Gold >= UpgradeCost && numMaterial >= UpgradeRequiredItems)
        {
            _equipment.LevelUp();

            Managers._Game.Gold -= UpgradeCost;

            Managers._Game.RemovMaterialItem(_equipment.EquipmentData.LevelupMaterialID, UpgradeRequiredItems);
            //Managers._Sound.Play(Define.Sound.Effect, "Levelup_Equipment");

            Refresh();
        }
        else
        {
            Managers._UI.ShowToast("재화가 부족합니다.");
        }
    ...
    void OnClickMergeButton() // 합성 버튼
    {
        ...
        UI_MergePopup mergePopupUI = (Managers._UI.SceneUI as UI_LobbyScene).MergePopupUI;
