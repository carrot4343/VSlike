...
public class UI_ToolTipItem : UI_Base
{
    ...
    {
        GetImage((int)Images.TargetImage).sprite = Managers._Resource.Load<Sprite>(materialData.SpriteName);
        GetText((int)Texts.TargetNameText).gameObject.SetActive(true); 
    ...
    // 몬스터 툴팁
    public void SetInfo(Data.MonsterData monsterData, RectTransform targetPos, RectTransform parentsCanvas)
    {
        /*GetImage((int)Images.TargetImage).sprite = Managers._Resource.Load<Sprite>(monsterData.IconLabel);
        GetText((int)Texts.TargetDescriptionText).gameObject.SetActive(true);
        GetText((int)Texts.TargetDescriptionText).text = monsterData.DescriptionTextID;
        GetImage((int)Images.BackgroundImage).color = EquipmentUIColors.Common;*/
    ...
    void OnClickCloseButton()
    {
        ...
        //자신 끄기
        Managers._Resource.Destroy(gameObject);
    }
